...
- **Working a slice** — `./mxcli brain brief --slice <name> -p WorkflowExample.mpr` emits
  exactly that pack (project + the slice's modules + its plan) as one read.
- **Planning, or picking work up** — run `./mxcli brain plan -p WorkflowExample.mpr`.
...
```bash
./mxcli -p WorkflowExample.mpr -c "SHOW STRUCTURE"   # one command
./mxcli exec script.mdl -p WorkflowExample.mpr        # a script
./mxcli                                          # REPL
```
...
```bash
./mxcli check script.mdl -p WorkflowExample.mpr --references   # syntax + references (~2s)
./mxcli exec script.mdl -p WorkflowExample.mpr                 # apply
./mxcli lint -p WorkflowExample.mpr                            # rules (~3s)
./mxcli report -p WorkflowExample.mpr                          # scored best practices
./mxcli docker check -p WorkflowExample.mpr                    # mxbuild, the slow one (~25s)
./mxcli run --local --watch -p WorkflowExample.mpr             # the app, hot-reloading
```
...
Set `mx` up once with `./mxcli setup mxbuild -p WorkflowExample.mpr`. To call it directly,
...
| Which lint rules exist | `./mxcli lint -p WorkflowExample.mpr --list-rules` |
| What a command takes | `./mxcli help <command>` |
| What this project contains | `./mxcli -p WorkflowExample.mpr -c "SHOW STRUCTURE"` |
