---
name: demo-data
description: "Seed a Mendix app's PostgreSQL database directly, bypassing the runtime — Mendix's internal ID system, association storage, and bulk `import from` an external database. Use when populating test or demo data, when data is needed before the UI exists, or for imports too large for the runtime."
---
...
> [verify-with-oql](../verify-with-oql/SKILL.md).
...
- [database-connections](../database-connections/SKILL.md) — Connecting to *external* databases from Mendix microflows
- [project-settings](../project-settings/SKILL.md) — Reading and changing project configuration with `alter settings`
- [generate-domain-model](../generate-domain-model/SKILL.md) — Creating entities before inserting data
